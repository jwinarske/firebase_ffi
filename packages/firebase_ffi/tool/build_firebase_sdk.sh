...
# toolchains, so this applies the patch set in ../patches — the same one the
# emb augment names, kept in one place rather than duplicated for CI. Both live
# inside the package so that a consumer who installed it from pub has them.
#
...
PATCH_ROOT="$PACKAGE_ROOT/patches/firebase-cpp-sdk"
