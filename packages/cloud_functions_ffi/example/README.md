...
# cloud_functions_ffi example

| | |
| --- | --- |
| [`lib/main.dart`](lib/main.dart) | Calls a callable named echo. `flutter run -d linux` |
| [`test/tour_test.dart`](test/tour_test.dart) | Arguments and results of every shape, failures, regions, and the streaming callables that are not bound. `flutter test` |

Both are ordinary `cloud_functions`; the `registerWith()` calls are the only
...
`products: [auth, database, functions]` here. Without one they say so at the first call.
