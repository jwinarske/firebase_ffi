...
# firebase_storage_ffi example

| | |
| --- | --- |
| [`lib/main.dart`](lib/main.dart) | Uploads a small object and downloads it again. `flutter run -d linux` |
| [`test/tour_test.dart`](test/tour_test.dart) | Upload as a task, download and its size cap, metadata, download URLs, deletion, and the listing that is not bound. `flutter test` |

Both are ordinary `firebase_storage`; the `registerWith()` calls are the only
...
`products: [auth, database, storage]` here. Without one they say so at the first call.
