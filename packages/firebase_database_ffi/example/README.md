...
# firebase_database_ffi example

| | |
| --- | --- |
| [`lib/main.dart`](lib/main.dart) | Writes a node and reads it back. `flutter run -d linux` |
| [`test/tour_test.dart`](test/tour_test.dart) | Writes, listeners, child events, queries, transactions, priorities, onDisconnect, offline writes, and the snapshot accessors that are not bound. `flutter test` |

Both are ordinary `firebase_database`; the `registerWith()` calls are the only
