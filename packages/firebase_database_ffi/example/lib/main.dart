...
// Writes a node and reads it back.
//
// The only lines specific to Linux are the registerWith calls below. Every
// other line is ordinary firebase_database and would compile unchanged on Android.
...
class _ExampleAppState extends State<_ExampleApp> {
  ...
  Future<String> _work() async {
    final ref = FirebaseDatabase.instance.ref('example/hello');
    await ref.set({'n': 1});
    final snap = await ref.get();
    return 'wrote and read back ${snap.value}';
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('firebase_database on Linux')),
        body: Center(
          child: Column(
